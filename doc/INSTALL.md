...
3.  Rename the environment variables template file `.env.template` in `.env` and configure the environment variables in the `.env` file. You can use `nano` for editing the config file.
    ...
    **Please note**:
    If you miss to provide some information the application start may fail or some features will not work properly.
    ...
    ***
    ...
    3.  Enter the **type** of your feeders. Currently supported: adsbx, airsquitter, dump1090-fa, fr24feeder, vrs (If you do not have a local feeder, just leave the value empty)
        ...
        ```
        FEEDER_TYPE=typeoffeeder1,typeoffeeder2
        ```
    ...
    9.  Opensky-Credentials: (**Optional**) Provide credentials with `OPENSKY_NETWORK_USERNAME` and `OPENSKY_NETWORK_PASSWORD`. If you don't provide this information this function will be disabled.
...
6.  Update standing data
    ...
    This will take some time. On an RaspberryPi 4B update database takes about 10-20 minutes.
